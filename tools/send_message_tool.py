                    ...
                    try:
                        if ext in _IMAGE_EXTS and not force_document:
                            ...
                            last_msg = await bot.send_document(
                                chat_id=int_chat_id, document=f, filename=display_name, **media_kwargs
                            )
...
def _check_send_message():
    ...
    Kanban workers normally report through ``kanban_complete`` so the parent
    gateway/notifier can deliver a single YURI-facing update. Older deployments
    allowed workers to use ``send_message`` directly; keep that path available
    only when it is not explicitly disabled by the dispatcher.
    """
    if os.environ.get("HERMES_KANBAN_TASK"):
        direct = os.environ.get("HERMES_KANBAN_ALLOW_DIRECT_SEND", "").strip().lower()
        if direct in {"0", "false", "no", "off"}:
            return False
        return True
    from gateway.session_context import get_session_env
