...
async def web_crawl_tool(
    ...
    """Crawl a site through a provider that supports ``crawl``.

    Tavily is the current crawl backend in our deployment.  It is intentionally
    budget-gated inside the provider via ``TAVILY_ALLOW_EXPENSIVE`` so the free
    monthly quota is not burned by accidental broad crawls.
    """
    try:
        if not is_safe_url(url):
            return json.dumps(
        ...
        policy_error = check_website_access(url)
        if policy_error:
            return json.dumps(
                {"success": False, "error": str(policy_error)},
                ensure_ascii=False,
            )

        backend = _get_backend() or _get_extract_backend()
        from agent.web_search_registry import get_provider as _wsp_get_provider

        provider = _wsp_get_provider(backend) if backend else None
        if provider is None or not hasattr(provider, "crawl"):
            return json.dumps(
                {
                    "success": False,
                    "error": (
                        "No web crawl provider configured. "
                        "Set web.backend or web.extract_backend to tavily."
                    ),
                },
                ensure_ascii=False,
            )
        ...
        crawl_fn = getattr(provider, "crawl")
        if inspect.iscoroutinefunction(crawl_fn):
            response = await crawl_fn(url, **crawl_kwargs)
        else:
            response = await asyncio.to_thread(crawl_fn, url, **crawl_kwargs)

        results = list((response or {}).get("results", []))
        ...
        return clean_base64_images(json.dumps(trimmed, indent=2, ensure_ascii=False))
    except Exception as e:
        logger.debug("Error crawling web: %s", e)
        return tool_error(f"Error crawling web: {str(e)}")
