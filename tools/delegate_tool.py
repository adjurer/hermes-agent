...
def _build_top_level_description() -> str:
    ...
    return (
        ...
        "For user-facing work, make the context a compact handoff brief instead "
        "of a raw transcript: goal, the previous 3-5 relevant messages summarized, "
        "constraints/non-goals, target files/URLs/servers, expected output, and "
        "the verification that must be returned.\n"
        ...
        "- Every delegated result should come back as one of: completed with "
        "verification, blocked with reason/evidence, or needs-review with the "
        "exact missing check. If the child returns a vague summary, treat it as "
        "not yet verified.\n"
