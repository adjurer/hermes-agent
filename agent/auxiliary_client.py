...
def call_llm(
    ...
    *,
    ...
    try:
        ...
        # Gemini OAuth is also treated as a soft auxiliary optimization on
        # rate limits so side tasks do not break the resident gateway during
        # short Code Assist capacity throttles.
        is_auto = resolved_provider in {"auto", "", None}
        is_gemini_rate_limit = (
            resolved_provider == "google-gemini-cli"
            and _is_rate_limit_error(first_err)
        )
        is_capacity_error = _is_payment_error(first_err) or _is_connection_error(first_err)
        if should_fallback and (is_auto or is_capacity_error or is_gemini_rate_limit):
...
async def async_call_llm(
    ...
    *,
    ...
    try:
        ...
        # fall through to the regular fallback chain.
        is_auto = resolved_provider in {"auto", "", None}
        is_gemini_rate_limit = (
            resolved_provider == "google-gemini-cli"
            and _is_rate_limit_error(first_err)
