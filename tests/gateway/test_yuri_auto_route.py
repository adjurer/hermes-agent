...
def _event(text: str, *, media: bool = False, is_bot: bool = False) -> MessageEvent:
    return MessageEvent(
        text=text,
...
def test_yuri_auto_route_complex_execution_becomes_pm_root():
    ...
    assert "reviewer는 처음 실행자가 아니라" in route["body"]
...
async def test_yuri_auto_route_persists_intake_context_for_short_followups(tmp_path, monkeypatch):
    ...
    assert "제가 이해한 일" in store.entries[1][1]["content"]
...
async def test_yuri_pm_root_creates_single_visible_root_task(tmp_path, monkeypatch):
    ...
    assert "맡아두겠습니다" in response
    assert "루트 업무" not in response
    conn = kb.connect()
    try:
